# ------------------------------------------------------------------------------
# name: tidysynth_3.R
# author: add some variables, try to fit better, 3rd attempt of synth
# description: synth 3rd attempt
...
library(haven)
library(tidyverse)
texas <- read_dta("D:/Xia Yu/???????ĵ?/????/textbook/Econ/Causal Inference/Assignment/Texas/Data/Clean/Texas_add_categories of crime.dta")%>%
  as.data.frame(.) 
...
texas_out <-
  ...
  generate_predictor(time_window = 1985:1993,
                     av_income = mean(income, na.rm = T)) %>% 
  generate_predictor(time_window = 1985:1993,
                     av_poverty = mean(poverty, na.rm = T)) %>% 
  generate_predictor(time_window = 1985:1993,
                     av_ur = mean(ur, na.rm = T)) %>%
  generate_predictor(time_window = 1985:1993,
                     av_pop = mean(pop, na.rm = T)) %>%
