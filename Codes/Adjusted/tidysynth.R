# ------------------------------------------------------------------------------
# name: tidysynth.R
# author: Yu Xia (UT Austin) 1st revised on scott cunningham (baylor) work on Github
# description: synth 1st attempt
...
texas_out <-
  ...
  generate_predictor(time_window = 1990:1992,
                     av_black = mean(black, na.rm = T)) %>% 
  generate_predictor(time_window = 1990,
                     av_aperc1519 = mean(perc1519, na.rm = T)) %>% 
